import { Controller,Post, Get, Body, UsePipes, ValidationPipe, UseGuards, Delete, Put, Param, UseInterceptors,
        UploadedFile, Bind, UploadedFiles, HttpException, HttpStatus, Req } from "@nestjs/common";
...
export class UserController {
    ...
    async getLoggedInUser(@Req() request: Request): Promise<User> {
        try {
            ...
            if(!userId) throw new HttpException('Invalid or Missing token', HttpStatus.BAD_REQUEST);
            return await this.usersService.getEmployeeById(Number(userId));
        }
        catch {
            throw new HttpException('Invalid or Missing token', HttpStatus.BAD_REQUEST);
        }
    }
    ...
    @Bind(UploadedFile())
    async changeUserPicture(@UploadedFile() picture: Express.Multer.File, @Param('id') id: number): Promise<User> {
    ...
    @Bind(UploadedFile())
    async deleteUser(@UploadedFile() document: Express.Multer.File, @Body() deleteUserDto: DeleteUserDTO): Promise<User> {
    ...
    async create(
        @UploadedFiles() files: { picture?: Express.Multer.File[], document?: Express.Multer.File[] },
        @Body() createUserDto: CreateUserDTO
    ): Promise<User> {
        return await this.usersService.create(createUserDto, files.picture, files.document);
    }
